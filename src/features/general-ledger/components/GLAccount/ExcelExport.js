...
const ExcelExport = ({data}) => {
  const createDownLoadData = () => {
    handleExport().then((url) => {
      ...
      downloadAnchorNode.setAttribute("download", "GL Account_report.xlsx");
  ...
  const handleExport = () => {
    ...
    const glAccount= row;
      table.push({
        A:glAccount.accountNo,
        B: glAccount.ArabicName,
        C: glAccount.EnglishName,
      });
    });
    ...
    XLSX.utils.book_append_sheet(wb, sheet, "GL Account_report");
    const workbookBlob = workbook2blob(wb);
    const headerIndexes = finalData.reduce((indexes, data, index) =>
    data["A"] === "ID" ? indexes.concat(index) : indexes, []);
